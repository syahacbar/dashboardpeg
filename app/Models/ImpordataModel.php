    ...
    public function uploadExcel()
    {
        ...
        foreach ($reader->getSheetIterator() as $sheet) {
            ...
            foreach ($sheet->getRowIterator() as $num_row => $row) {
                ...
                $total_row += 1;
                if ($num_row % 2000 == 0) {
                    $query = $this->db->table($this->request->getPost('tabel_impordata'))->insertBatch($data_db);
                    $data_db = [];
                }
            }
        ...
        if ($query) {
            $result['status'] = 'ok';
            $result['content'] = 'Data berhasil di masukkan ke dalam tabel <strong>' . $_POST['tabel_impordata'] . '</strong> sebanyak ' . format_ribuan($total_row) . ' baris';
        }
